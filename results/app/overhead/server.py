...
def _transport_name(value) -> str:
    return value.value if hasattr(value, "value") else str(value)


def _build_http_health_url(http_base_url: str) -> str:
    return http_base_url.rstrip("/") + "/health"


def _build_ws_health_url(ws_url: str) -> str:
...
def _assert_transports_ready_for_request_response(config) -> None:
    failures: list[str] = []
    ...
    for transport in config.transports:
        ...
        if name == "http":
            status = check_http_like_service(
                service_name="http_transport",
                url=_build_http_health_url(config.http_base_url),
                timeout_seconds=1.5,
            )
        elif name == "ws":
            status = check_http_like_service(
                service_name="ws_transport",
                url=_build_ws_health_url(config.ws_url),
                timeout_seconds=1.5,
            )
        else:
            status = check_grpc_service(
    ...
    if failures:
        raise HTTPException(
            status_code=503,
            detail=(
                "Eksperyment nie może zostać uruchomiony. "
                "Niedostępne transporty: " + "; ".join(failures)
            ),
        )


def _assert_transports_ready_for_realtime(config) -> None:
    failures: list[str] = []
    ...
    for transport in config.transports:
        ...
        if name == "http":
            ...
            status = check_http_like_service(
                service_name="ws_transport",
                url=_build_ws_health_url(config.ws_process_url),
                timeout_seconds=1.5,
            )
        else:
            status = check_grpc_service(
...
def _assert_transports_ready_for_serialization(config) -> None:
    failures: list[str] = []
...
def run_request_response_endpoint(
    ...
    _assert_transports_ready_for_request_response(config)
...
def run_realtime_endpoint(
    ...
    _assert_transports_ready_for_realtime(config)
...
def run_serialization_endpoint(
    ...
    _assert_transports_ready_for_serialization(config)
