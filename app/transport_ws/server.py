...
from app.core.models.realtime import StateChangeRequest, StateChangeWsResponse
...
async def websocket_changes_endpoint(websocket: WebSocket) -> None:
    await websocket.accept()
    ...
    try:
        while True:
            ...
            try:
                request = StateChangeRequest.model_validate(raw_data)
                batch = get_state_changes_since(state_store, request.from_version)

                response = StateChangeWsResponse(batch=batch)
...
@app.post("/ws/changes/seed")
def seed_changes_endpoint() -> dict[str, int]:
