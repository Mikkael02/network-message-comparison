...
CSV_INPUT_PATH = Path("results/processed/realtime_fetch_summary.csv")
...
def load_rows(path: Path) -> list[dict]:
    ...
    for row in rows:
        ...
        row["expected_event_count"] = int(row["expected_event_count"])
...
def plot_metric(rows: list[dict], scenario: str, metric: str, output_path: Path) -> None:
...
def main() -> None:
    ...
    for scenario in ["non_empty_fetch", "empty_fetch"]:
        scenario_rows = filter_rows(rows, scenario)

        avg_output = OUTPUT_DIR / f"{scenario}_avg_ms.png"
        median_output = OUTPUT_DIR / f"{scenario}_median_ms.png"

        plot_metric(scenario_rows, scenario, "avg_ms", avg_output)
        plot_metric(scenario_rows, scenario, "median_ms", median_output)
